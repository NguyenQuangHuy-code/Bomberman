...
public class Sprite {
    ...
    //Doll
    public static Sprite enemy_left1 = new Sprite(DEFAULT_SIZE, 7, 6, SpriteSheet.tiles, 14, 16);
    ...
    public Image getFxImage() {
        ...
        for (int x = 0; x <(SIZE - _realWidth) / 2; ++x) {
            for (int y =0 ; y < SIZE; ++y) {
                pw.setArgb(x, y, 0);
            }
        }
        for (int x = (SIZE - _realWidth) / 2; x < SIZE; x++) {
            for (int y = 0; y < SIZE; y++) {
                if ( _pixels[x - (SIZE - _realWidth) / 2 + y * SIZE] == TRANSPARENT_COLOR) {
                    pw.setArgb(x, y, 0);
                }
                else {
                    pw.setArgb(x, y, _pixels[x - (SIZE - _realWidth) / 2 + y * SIZE]);
                }
            }
        }
